...
import {IOpticApiSnapshotRequest} from './index'

type Credentials = { token: string }

class Client {
  private readonly getCredentials: () => Credentials | PromiseLike<Credentials>
  private readonly baseUrl: string

  constructor(baseUrl: string, getCredentials: () => Credentials | PromiseLike<Credentials>) {
  ...
  postSelfApiSnapshotByApiSlug(apiSlug: string, selfApiSnapshot: IOpticApiSnapshotRequest) {
    return Promise
      .resolve(this.getCredentials())
      .then((_credentials: { token: string }) => {
        return unirest
